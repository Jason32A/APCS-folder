public class DriverForFood
{
    public static void foodItemTester()
    {
        ...
        for (int x = 0; x < items.length; x++)
            System.out.println("Item #" + items[x].getID() + " Profit: " + 
    ...
    public static void cashRegisterTester()
    {
        ...
        for (int regIndex = 0; regIndex < regArr.length; regIndex++)
        {
            ...
            {
                if (itemArr[itemIndex].getUnitsInStock() < itemIndex + regIndex)
                    regArr[regIndex].scanItem(itemArr[itemIndex], itemArr[itemIndex].getUnitsInStock());
                else
        ...
        System.out.println("Total FoodItem Rev:  $" + ((int)(100*FoodItem.getTotalRev())/100.0));
